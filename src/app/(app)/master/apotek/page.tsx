...
const emptyForm = { nama: '', alamat: '', noTelp: '', pemilik: '', pic: '', aktif: true };

export default function ApotekPage() {
  const [data, setData] = useState<Apotek[]>([]);
  ...
  const openEdit = (item: Apotek) => {
    setEditId(item.id);
    setForm({ nama: item.nama, alamat: item.alamat, noTelp: item.noTelp, pemilik: item.pemilik, pic: item.pic, aktif: item.aktif });
    setShowModal(true);
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Yakin ingin menghapus apotek ini?')) return;
    await fetch(`/api/apotek/${id}`, { method: 'DELETE' });
    fetchData();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editId ? `/api/apotek/${editId}` : '/api/apotek';
    await fetch(url, {
      method,
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 md:mb-6">
        <h1 className="text-2xl font-bold text-white">Master Apotek</h1>
        <button
          onClick={openAdd}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                ...
                <th className="px-4 py-3 text-left text-gray-400 font-medium">Pemilik</th>
                <th className="px-4 py-3 text-left text-gray-400 font-medium">PIC</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-4 py-8 text-center text-gray-500">Tidak ada data</td>
                </tr>
              ) : (
                data.map((item, idx) => (
                  <tr
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-300">{item.pemilik}</td>
                    <td className="px-4 py-3 text-gray-300">{item.pic}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-md">
            <div className="p-6">
              <h2 className="text-lg font-semibold text-white mb-4">{editId ? 'Edit Apotek' : 'Tambah Apotek'}</h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Pemilik</label>
                  <input
                    type="text"
                    value={form.pemilik}
                ...
                <div>
                  <label className="block text-sm text-gray-400 mb-1">PIC</label>
                  <input
                    type="text"
                    value={form.pic}
