...
interface StokKonsinyasi { produkId: string; jumlah: number; }
...
interface ItemRow { produkId: string; jumlah: number; }
...
export default function KirimKonsinyasiPage() {
  ...
  const [editItem, setEditItem] = useState<KirimLog | null>(null);
  const [editForm, setEditForm] = useState({ jumlah: 0, keterangan: '', tanggal: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/konsinyasi/kirim', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ apotekId, salesId, keterangan, tanggal, items }) });
      const data = await res.json();
      if (data.success) {
        setMessage({ type: 'success', text: data.message || `${items.length} produk berhasil dikirim` });
        setItems([{ ...emptyItem }]); setKeterangan(''); fetchData();
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Hapus data kirim ini? Stok akan dikembalikan.')) return;
    setMessage({ type: '', text: '' });
    try {
      const res = await fetch('/api/konsinyasi/kirim', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
      const data = await res.json();
      if (data.success) { setMessage({ type: 'success', text: 'Data kirim dihapus, stok dikembalikan' }); fetchData(); }
      else { setMessage({ type: 'error', text: data.message || 'Gagal menghapus' }); }
    } catch { setMessage({ type: 'error', text: 'Terjadi kesalahan' }); }
  };

  const openEdit = (item: KirimLog) => {
    setEditItem(item);
    setEditForm({ jumlah: item.jumlah, keterangan: item.keterangan, tanggal: item.tanggal.slice(0, 10) });
  };
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { setMessage({ type: 'success', text: 'Data kirim berhasil diupdate' }); setEditItem(null); fetchData(); }
      ...
      <form onSubmit={handleSubmit}>
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-4 md:p-6 mb-4 md:mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
            <div><label className="block text-xs md:text-sm text-gray-400 mb-1">Tanggal</label><input type="date" value={tanggal} onChange={(e) => setTanggal(e.target.value)} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" required /></div>
            <div><label className="block text-xs md:text-sm text-gray-400 mb-1">Apotek</label><SearchSelect options={apotekList.map((a) => ({ value: a.id, label: a.nama }))} value={apotekId} onChange={setApotekId} placeholder="Pilih Apotek" /></div>
            <div><label className="block text-xs md:text-sm text-gray-400 mb-1">Sales</label><SearchSelect options={salesList.map((s) => ({ value: s.id, label: s.nama }))} value={salesId} onChange={setSalesId} placeholder="Pilih Sales" /></div>
            <div className="sm:col-span-2 lg:col-span-2"><label className="block text-xs md:text-sm text-gray-400 mb-1">Keterangan</label><input type="text" value={keterangan} onChange={(e) => setKeterangan(e.target.value)} placeholder="Opsional" className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
          </div>
        </div>

        {apotekId && (
          <div className="bg-slate-900/50 rounded-xl border border-indigo-500/20 p-4 mb-4 md:mb-6">
            <div className="flex items-center gap-2 mb-3"><span className="text-indigo-400 font-medium text-sm">Stok Konsinyasi di Apotek</span>{loadingStok && <span className="text-xs text-gray-500">Memuat...</span>}</div>
            {stokKonsinyasi.length === 0 ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                {stokKonsinyasi.map((stok) => {
                  ...
                  return (
                    <div key={stok.produkId} className={`px-3 py-2 rounded-lg border text-sm ${stok.jumlah <= 5 ? 'bg-red-500/5 border-red-500/20' : stok.jumlah <= 15 ? 'bg-yellow-500/5 border-yellow-500/20' : 'bg-green-500/5 border-green-500/20'}`}>
                      <div className="text-gray-300 font-medium truncate">{produk.nama}</div>
                      <div className={`font-bold ${stok.jumlah <= 5 ? 'text-red-400' : stok.jumlah <= 15 ? 'text-yellow-400' : 'text-green-400'}`}>{stok.jumlah} {produk.satuan}</div>
                    </div>
                  );
                })}
              </div>
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-4 md:p-6 mb-4 md:mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button type="button" onClick={addItem} className="px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs md:text-sm font-medium transition-colors">+ Tambah Baris</button>
          </div>
          <div className="space-y-3">
            <div className="hidden md:grid grid-cols-12 gap-2 text-sm text-gray-400 font-medium px-1">
              <div className="col-span-4">Produk</div><div className="col-span-2">Stok Gudang</div><div className="col-span-2">Jumlah</div><div className="col-span-3"></div><div className="col-span-1"></div>
            </div>
            {items.map((item, idx) => {
              const stok = getStokGudang(item.produkId);
              ...
              return (
                <div key={idx}>
                  <div className="hidden md:grid grid-cols-12 gap-2 items-center">
                    <div className="col-span-4"><SearchSelect options={produkList.map((p) => ({ value: p.id, label: `${p.nama} (${p.satuan})` }))} value={item.produkId} onChange={(val) => updateItem(idx, 'produkId', val)} placeholder="Pilih Produk" /></div>
                    <div className="col-span-2 text-center">{item.produkId ? <span className={`text-sm font-semibold ${isOverStok ? 'text-red-400' : stok <= 5 ? 'text-yellow-400' : 'text-gray-300'}`}>{stok.toLocaleString('id-ID')}</span> : <span className="text-sm text-gray-600">-</span>}</div>
                    <div className="col-span-2"><input type="number" value={item.jumlah} onChange={(e) => updateItem(idx, 'jumlah', Number(e.target.value))} className={`w-full px-2 py-2 bg-gray-800 border rounded-lg text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isOverStok ? 'border-red-500/50' : 'border-gray-700'}`} min={1} required /></div>
                  ...
                  <div className="md:hidden bg-gray-800/50 rounded-lg p-3 border border-gray-700/50 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div><label className="text-xs text-gray-400">Stok Gudang</label><div className={`text-sm font-semibold ${isOverStok ? 'text-red-400' : stok <= 5 ? 'text-yellow-400' : 'text-gray-300'}`}>{item.produkId ? stok.toLocaleString('id-ID') : '-'}</div></div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                ...
                <th className="px-3 md:px-4 py-3 text-center text-gray-400 font-medium">Aksi</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
      ...
      {editItem && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setEditItem(null)}>
          <div className="bg-gray-900 rounded-xl border border-gray-700 p-6 w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-white mb-4">Edit Kirim Konsinyasi</h3>
            <div className="space-y-3">
              <div><p className="text-sm text-gray-400">Produk</p><p className="text-white">{editItem.produk.nama}</p></div>
              <div><label className="block text-sm text-gray-400 mb-1">Tanggal</label><input type="date" value={editForm.tanggal} onChange={(e) => setEditForm({ ...editForm, tanggal: e.target.value })} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100" /></div>
              <div><label className="block text-sm text-gray-400 mb-1">Jumlah</label><input type="number" value={editForm.jumlah} onChange={(e) => setEditForm({ ...editForm, jumlah: Number(e.target.value) })} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100" min={1} /></div>
              <div><label className="block text-sm text-gray-400 mb-1">Keterangan</label><input type="text" value={editForm.keterangan} onChange={(e) => setEditForm({ ...editForm, keterangan: e.target.value })} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100" /></div>
            </div>
