...
const sortOptions = [
  ...
  { value: 'totalTerjual:desc', label: 'Terjual Terbanyak' },
];
...
export default function PerputaranPage() {
  const [data, setData] = useState<PerputaranItem[]>([]);
  ...
  const fetchData = useCallback(() => {
    ...
    fetch(`/api/laporan/perputaran?${params.toString()}`)
      .then((res) => res.json())
      .then((d) => {
  ...
  const handleExport = () => {
    exportCsv('laporan-perputaran.csv', ['No', 'Nama Produk', 'Kategori', 'Total Terjual', 'Total Revenue', 'Rata-rata/Bulan'], data.map((item, i) => [
      i + 1, item.nama, item.kategori, item.totalTerjual, Math.round(item.totalRevenue), item.rataRataPerBulan.toFixed(1),
    ]));
  };
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">Laporan Perputaran</h1>
          <p className="text-gray-400 text-sm mt-1">Analisis perputaran dan penjualan produk</p>
        </div>
        <button
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                ...
                <th className="px-4 py-3 text-right text-gray-400 font-medium">Rata-rata/Bulan</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span>Tidak ada data perputaran ditemukan</span>
                ...
                data.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-300 text-right">{item.rataRataPerBulan.toFixed(1)}</td>
                  </tr>
                ))
              )}
            </tbody>
