...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await prisma.$queryRaw<{
      ...
      totalJumlah: number; totalMargin: number; totalRevenue: number; totalHpp: number; persentaseMargin: number;
    }[]>`
      SELECT
        dp."produkId",
        pr.nama,
    ...
    return NextResponse.json({
      ...
      data: data.map((d) => ({
        ...d,
        totalJumlah: Number(d.totalJumlah),
